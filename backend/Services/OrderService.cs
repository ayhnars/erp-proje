...
public class OrderService : IOrderService
{
    ...
    {
        var order = await _orders.GetByIdAsync(dto.OrderID);  // Order modelindeki anahtar adına göre: Id/OrderId olabilir
        if (order == null)
        ...
        foreach (var i in dto.Items)
        {
            if (i.ItemID == 0)
            {
                ...
                newItem.OrderID = dto.OrderID;   // OrderID alan adın farklıysa (OrderId) uyumla
